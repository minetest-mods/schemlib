...
--------------------------------------
function plan_class:add_mts_obj(mts, plan_os, replacements)
	local size = mts.size
	local size_x = size.x
	local size_y = size.y
	local size_z = size.z
	if type(plan_os) ~= "table" then plan_os = {} end
	local ox = plan_os['x'] or 0
	local oy = plan_os['y'] or 0
	local oz = plan_os['z'] or 0
